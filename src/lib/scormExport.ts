...
function parseModules(archRaw: string): Module[] {
  ...
  if (data?.modules) return data.modules.map((m: any) => ({ title: String(m.title || m.module_title || ""), topics: (m.topics || m.sections || []).map((t: any) => typeof t === "string" ? t : t.topic_name || t.topic_title || t.title || t.topic || t.name || JSON.stringify(t)) }));
