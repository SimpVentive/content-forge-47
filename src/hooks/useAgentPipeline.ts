...
export function useAgentPipeline() {
  ...
  const runPipeline = useCallback(async (courseTitle: string, inputText: string, toggles: Record<string, boolean>, params?: { level?: string; language?: string; textLanguage?: string; narratorLanguage?: string; voiceAccent?: string; duration?: string; assessmentRequired?: boolean; assessmentTypes?: ("mcq" | "match-the-following" | "true-false" | "scenario" | "fill-blanks")[]; assessmentIntensity?: AssessmentIntensity; slideLayout?: SlideLayoutParams; maxYoutubeVideos?: number; learningMode?: VideoMode; videoSettings?: { selectedAvatar: string; videoQuality: string; backgroundStyle: string }; imageCount?: 1 | 2 | 3 | 4 | 5; imageNarrativeSceneCount?: number; imageStyleVariant?: string; imageAspectRatio?: string; characterEthnicity?: string; avatarTrainerId?: string; flipbookDisplayStyle?: "page-flip" | "smooth-slide" | "step-reveal"; imageOutputFormat?: "interactive-html" | "video" | "pdf"; flipbookVoiceoverEnabled?: boolean; flipbookNarrationLanguage?: string; showAvatarNarrator?: boolean; voiceoverPace?: "slow" | "normal" | "fast"; contentType?: "learning-course" | "work-instruction"; titleSpans?: any[]; companyLogo?: string | null }, selectedAssets?: SelectedAsset[], courseId?: string, userId?: string) => {
    ...
    const isVideoMode = learningMode !== "static_elearning" && learningMode !== "image_based_learning";
...
- You MUST respect the supplied runtime plan. Hit the requested word budgets closely so the narration lands within ${durationToleranceMinutes} minutes of the ${durationMinutes}-minute target.
      ...
      console.log("Visual Narrative check: learningMode =", learningMode, "toggles['visual-narrative'] =", toggles["visual-narrative"]);
      ...
      if (toggles["youtube"] !== false) {
        ...
        try {
        const modeInstructions = getAgentModeInstructions("youtube", learningMode);
        const { data, error } = await supabase.functions.invoke("youtube-search", {
            body: { modules: moduleNames, courseTitle, language: params?.language, level: params?.level, duration: params?.duration, maxVideos: params?.maxYoutubeVideos ?? 20, learningMode, modeInstructions },
          });
      ...
      if (toggles["compliance"] !== false) {
        ...
        const modeInstructions = getAgentModeInstructions("compliance", learningMode);
        complianceResult = await runAgentWithLanguage(
          `${languageDirective}You are a Compliance Agent for eLearning content. Review the course script and outline for the following: (1) Reading level — is it appropriate for the target audience? (2) Inclusive language — flag any non-inclusive terms. (3) Accessibility — does content support screen readers, captions, alt-text needs? (4) Policy alignment — does it reference relevant standards (OSHA, WCAG 2.1, etc.)? (5) Overall compliance score out of 10. \n\n[LEARNING MODE: ${learningMode}]\n${modeInstructions}\n\nReturn as JSON: { reading_level, inclusive_language_issues: [], accessibility_notes: [], policy_alignment: [], compliance_score, recommendations: [] }`,
          `Script:\n${writerResult}`,
      ...
      if (toggles["assessment"] !== false) {
        ...
        const selectedAssessmentTypes = params?.assessmentTypes || ["mcq", "scenario"];
        const assessmentTypeList = selectedAssessmentTypes.map(t => {
          const typeMap: Record<string, string> = {
            "mcq": "Multiple Choice Questions (MCQ)",
            "true-false": "True/False questions",
            "match-the-following": "Match the Following (pair matching)",
            "scenario": "Scenario-based questions",
            "fill-blanks": "Fill in the Blanks (short answer)",
          };
          return typeMap[t] || t;
        }).join(", ");
        const hasScenarios = selectedAssessmentTypes.includes("scenario");
        const hasMCQ = selectedAssessmentTypes.includes("mcq");

        addLog(`Assessment Agent: Generating assessments (${assessmentTypeList}) at ${assessmentIntensity} intensity...`);
        const modeInstructions = getAgentModeInstructions("assessment", learningMode);
        const assessmentTypeInstructions = `ASSESSMENT TYPES TO GENERATE:\nThe course will use ONLY these assessment types: ${assessmentTypeList}.\nDo NOT generate assessment types not listed above.\nGenerate questions ONLY in the selected types.`;
        const baseAssessmentPrompt = `You are an Assessment Design Agent. Given course script, architecture, and learning objectives, create a comprehensive assessment bank sized to the requested runtime and module complexity. LANGUAGE REQUIREMENT: Write ALL question text, options, rationale, situation descriptions, prompts, and every text field in ${textLanguage}. Do not use English if ${textLanguage} is not English. Rules: (1) Question volume must scale with module/topic size; avoid token quizzes. (2) Spread questions across modules proportionally. (3) Include module_title and topic_title on every question so the renderer can map items correctly. (4) ${assessmentTypeInstructions} LEVEL CALIBRATION (${levelCalibration.label}): ${assessmentLevelGuidance} Bloom's target for questions: ${levelCalibration.bloomsTarget} \n\n[LEARNING MODE: ${learningMode}]\n${modeInstructions}\n\nGenerate: ${selectedAssessmentTypes.includes("mcq") ? "(1) requested number of multiple choice questions with 4 options each, correct answer marked, rationale, and common wrong-answer trap, " : ""}${selectedAssessmentTypes.includes("true-false") ? "(2) requested number of true/false questions with correct answer and rationale, " : ""}${selectedAssessmentTypes.includes("match-the-following") ? "(3) match-the-following exercises with column A items, column B items, correct pairings, and explanation, " : ""}${selectedAssessmentTypes.includes("scenario") ? `(${hasMCQ || selectedAssessmentTypes.includes("true-false") ? "4" : "2"}) scenario-based questions with situation description, 3 response options, best_response, and coaching rationale, ` : ""}${selectedAssessmentTypes.includes("fill-blanks") ? "(5) fill-in-the-blanks questions with passage, blank indicator, correct answer, and rationale, " : ""}(${selectedAssessmentTypes.length + 1}) 1 reflection exercise with an open-ended prompt, and (${selectedAssessmentTypes.length + 2}) embedded_interactions list in requested range aligned to specific topics. Tag each question with Bloom's taxonomy level. Return JSON: { ${selectedAssessmentTypes.includes("mcq") ? "mcq: [{ module_title, topic_title, question, options: [], correct_answer, rationale, wrong_answer_trap, blooms_level }], " : ""}${selectedAssessmentTypes.includes("true-false") ? "true_false: [{ module_title, topic_title, question, correct_answer, rationale, blooms_level }], " : ""}${selectedAssessmentTypes.includes("match-the-following") ? "match_the_following: [{ module_title, topic_title, column_a: [], column_b: [], correct_pairings: [], explanation, blooms_level }], " : ""}${selectedAssessmentTypes.includes("scenario") ? "scenarios: [{ module_title, topic_title, situation, options: [], best_response, rationale, blooms_level }], " : ""}${selectedAssessmentTypes.includes("fill-blanks") ? "fill_blanks: [{ module_title, topic_title, passage, correct_answer, rationale, blooms_level }], " : ""}reflection: { prompt, guidance }, embedded_interactions: [{ module_title, topic_title, interaction_type, prompt, expected_response, feedback_focus }], metadata: { assessment_types: [${selectedAssessmentTypes.map(t => `"${t}"`).join(", ")}], module_count, topic_count, assessment_intensity } }`;
      ...
      if (toggles["voice"] !== false) {
        ...
        const narratorInfo = params?.avatarTrainerId ? `\nNARRATOR SPECIFICATION:\n- Selected Avatar/Trainer: ${params.avatarTrainerId}\n- Character Ethnicity: ${params?.characterEthnicity || 'diverse'}\n- CRITICAL: Voice must match the selected narrator. Do NOT override or ignore this specification.` : "";
        const voiceSystemPrompt = `${languageDirective}You are a Voice and Narration Agent. Given a course script, reformat it as a professional narration script optimised for text-to-speech or voice recording. The narration language must be ${narratorLanguage}. If the source script is in a different language, translate it naturally while preserving meaning. For each section: (1) rewrite the script with natural spoken-word phrasing (shorter sentences, contractions, conversational), (2) add SSML-style narration cues in brackets like [PAUSE 1s], [EMPHASIZE], [SLOW DOWN], (3) estimate word count and approximate read time at ${NARRATION_WORDS_PER_MINUTE} words per minute. Preserve section structure, and keep the overall read-time within +/- ${durationToleranceMinutes} minutes of the ${durationMinutes}-minute target unless the script makes that impossible. Return the full narration script with cues and a summary: { total_words, estimated_duration_minutes, sections: [{ title, narration_text, word_count }] }${narratorInfo}${isVideoMode ? `\n\n[EXECUTION MODE: ${learningMode}]\n${getAgentModeInstructions("voice_narration", learningMode as any)}` : ""}`;
